...
class CareerPageScraper:
    ...
    def scrape_jobs(self, career_url: str, company_name: str, source_opportunity: OpportunityData) -> List[OpportunityData]:
        """
    ...
    def _extract_jobs(self, soup: BeautifulSoup, career_url: str, company_name: str, source_opportunity: OpportunityData) -> List[OpportunityData]:
    ...
    def _extract_job_location(self, link_element, soup: BeautifulSoup) -> str:
        ...
        if parent:
            ...
            location_match = re.search(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*,\s*[A-Z]{2}|Remote|Hybrid)\b', text)
            if location_match:
