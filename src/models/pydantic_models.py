...
Key Validations:
- Seniority scoring weights must sum to ≤30 points
...
class ProfileConfig(BaseModel):
    ...
    def validate_digest_score_consistency(self) -> "ProfileConfig":
        """Ensure digest min_score and min_grade are consistent"""
        # Grade thresholds: A=85+, B=70+, C=55+, D=40+, F=<40
        grade_thresholds = {"A": 85, "B": 70, "C": 55, "D": 40, "F": 0}

        digest_grade = self.digest.min_grade
        digest_score = self.digest.min_score
        expected_min_score = grade_thresholds[digest_grade]

        if digest_score < expected_min_score:
            raise ValueError(
                f"digest.min_score ({digest_score}) is lower than expected for grade {digest_grade} "
                f"(expected ≥{expected_min_score})"
            )

        return self

    @model_validator(mode="after")
    def validate_notification_score_consistency(self) -> "ProfileConfig":
        """Ensure notification min_score and min_grade are consistent"""
        # Grade thresholds: A=85+, B=70+, C=55+, D=40+, F=<40
        grade_thresholds = {"A": 85, "B": 70, "C": 55, "D": 40, "F": 0}

        notif_grade = self.notifications.min_grade
        notif_score = self.notifications.min_score
        expected_min_score = grade_thresholds[notif_grade]

        if notif_score < expected_min_score:
            raise ValueError(
                f"notifications.min_score ({notif_score}) is lower than expected for grade {notif_grade} "
                f"(expected ≥{expected_min_score})"
            )
