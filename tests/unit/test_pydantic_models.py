...
- Custom validators (seniority, grades, etc.)
...
class TestProfileConfig:
    ...
    def test_digest_score_grade_inconsistency_fails(self):
        """Test digest min_score inconsistent with min_grade fails"""
        with pytest.raises(ValidationError) as exc_info:
            ProfileConfig(
                id="test",
                name="Test",
                ...
                digest=Digest(
                    min_grade="B",  # B grade requires ≥70
                    min_score=50,  # But score is only 50
                    include_grades=["A", "B"],
                ),
                notifications=Notifications(enabled=False, min_grade="A", min_score=85),
            )
        assert "digest.min_score" in str(exc_info.value)
        assert "lower than expected for grade B" in str(exc_info.value)

    def test_notification_score_grade_inconsistency_fails(self):
        """Test notification min_score inconsistent with min_grade fails"""
        with pytest.raises(ValidationError) as exc_info:
            ProfileConfig(
                id="test",
                name="Test",
                ...
                notifications=Notifications(
                    enabled=True,
                    min_grade="A",  # A grade requires ≥85
                    min_score=70,  # But score is only 70
                ),
            )
        assert "notifications.min_score" in str(exc_info.value)
        assert "lower than expected for grade A" in str(exc_info.value)
...
class TestRealProfileValidation:
    ...
    def test_adam_profile_structure(self):
        ...
        config = ProfileConfig(
            ...
            digest=Digest(
                min_grade="C", min_score=55, include_grades=["A", "B", "C"], send_frequency="weekly"
            ),
            notifications=Notifications(enabled=True, min_grade="B", min_score=70),
        )
    ...
    def test_eli_profile_structure(self):
        ...
        config = ProfileConfig(
            ...
            notifications=Notifications(enabled=False, min_grade="B", min_score=70),
        )
